...
| [docs/design-name-event-log.md](docs/design-name-event-log.md) | Proposed: per-name append-only event log under `events/<name>/<ulid>.toml` recording lifecycle transitions (claim, release, force-release, start/stop, fork, rename). Pointer stays canonical for "now"; log is canonical for "ever". Includes a rename design that ties two name logs together without copying history |
