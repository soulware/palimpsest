...
| [docs/design-gc-bucket-unification.md](docs/design-gc-bucket-unification.md) | Proposed: collapse GC's smalls + one-filler tiers into a single bin-pack that emits N output buckets per tick; mirrors #297's pending unification. Body resolution is cache-first under plan-handoff, so the per-tick cap (`max_buckets_per_tick`, default 4) is sized against retention-peak rather than S3 bandwidth |
