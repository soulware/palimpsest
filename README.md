...
| [docs/design-manifest-toml-removal.md](docs/design-manifest-toml-removal.md) | Proposed: drop `manifest.toml` entirely. `name` / `readonly` / `origin` are redundant with existing surfaces; OCI `source` migrates onto signed `volume.provenance` as `oci_source`. Fresh-bucket-only |
