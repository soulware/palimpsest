...
| [docs/design-segment-index.md](docs/design-segment-index.md) | Proposed: P3 (folding P4) of list elimination — replace the per-vol `segments/`+`retention/` LISTs with a *second manifest* (`by_id/<vol>/tail`, single overwritten object) for the post-snapshot delta over P2's `LATEST` anchor. Reaper folded into the per-volume tick loop → sole sequential writer, one GET reads any-size tail, no lock/chain. Cross-coordinator only (owner uses local `index/`); derived/unsigned over a per-segment-signed substrate |
...
| [docs/list-elimination-plan.md](docs/list-elimination-plan.md) | Plan: remove all `s3:ListBucket` use from the coordinator runtime — replace each per-volume/event prefix LIST with a deterministic GET (latest-pointer or maintained index), then delete the grant from `coord-writer`. Resolves design-mint open #12. Phased P1–P5; no-LIST reconcile story. |
