...
| [docs/portable-live-volume-plan.md](docs/portable-live-volume-plan.md) | Plan: phased implementation of portable live volumes (foundations → schema → lifecycle verbs → force-takeover → CLI unification → tests/docs). Fresh-bucket-only; clean break for `volume remote` |
