...
| [docs/design-elide-mint.md](docs/design-elide-mint.md) | Proposed: `elide-mint` sibling process holds the Tigris admin credential; coord runs without `AWS_*` admin env. Topology is strictly `volume ↔ coord ↔ mint` — volumes never speak to mint. Single supervisor (`elide coord run`) manages both children in standalone and under one systemd unit (Postfix-style: `master(8)` supervises role-specific daemons inside one `postfix.service`). coord↔mint authenticated by filesystem permissions + peer-cred uid + a supervisor-generated startup RPC token (the layer that keeps same-uid volume processes out). Mint exposes a small typed IAM-API wrapper (`mint_writer_key`, `mint_ro_key`, `refresh_ro_key`, etc); no macaroon handling. Concretises the per-host credentialer sidecar option from `design-iam-key-model.md` § *Admin key containment* |
