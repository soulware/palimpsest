...
The fetch worker uses the same `LazyCredsFetcher` flow the volume daemon uses, with two differences:

- **PID file is `fetch.pid`, not `volume.pid`.** The orchestrator plants `<by_id>/<vol_ulid>/fetch.pid` with the worker's PID immediately after `cmd.spawn()` returns. An RAII guard in the orchestrator removes it on every exit path (success, failure, panic). Lifecycle classifier doesn't look at this file — separating it from `volume.pid` keeps a fetched volume from being mis-classified as `Running` while the worker is alive.
- **Macaroon is `Scope::FetchWorker`, not `Scope::Credentials`.** Distinct IPC verb (`Request::RegisterFetchWorker`) checks SO_PEERCRED against `fetch.pid` and stamps the issued macaroon with `Scope::FetchWorker`. The `Credentials` IPC accepts both scopes, so the lazy-creds wrapper's idle re-issue path works uniformly. The scope separation guards against a leaked fetch macaroon being replayed at some future verb that wants to gate on "is this a real volume daemon."

Race window: the worker's macaroon registration can land before the orchestrator's `fetch.pid` write completes. The volume-daemon path already absorbs this with `register_volume_with_retry`'s "not registered" backoff loop; the fetch worker reuses the same retry shape via `register_fetch_worker_with_retry`.

Trust model: the coordinator owns spawn — anything claiming to be a fetch worker must race the coordinator to plant a `fetch.pid` matching its own PID, which it cannot do unless it already has write access to `by_id/<vol_ulid>/`. The macaroon adds a cryptographic binding on top so a leak in transit doesn't compromise other volumes' creds.
