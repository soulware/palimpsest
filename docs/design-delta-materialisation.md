...
4. **Interaction with sparse strategy (see [architecture.md](architecture.md) — sparse vs delta).** Sparse-uploaded segments don't carry delta entries in the canonical form — their format-level entries are full DATA. `.dmat` is irrelevant for sparse segments and is never created for them. No interaction, but worth restating to avoid confusion.
