...
- `<snap>.filemap` — per-file fragment table from `snapshots/<snap>.filemap`; served on demand for operator tooling (`elide volume import --extents-from`, `elide volume generate-filemap`) but **not** prefetched by claim/start. Filemaps are consumed only by the import path, are intentionally absent for handoff snapshots produced by `release` (no ext4 layout to derive from), and the import path already tolerates a missing source filemap by skipping that source. Pulling them speculatively during prefetch wasted bandwidth on every claim and surfaced fatal 404s on handoff snapshots. Unsigned; row-level blake3 hashes are content-verified at import time, so a tampered filemap can mislead dedup lookup but cannot let corrupted bytes through.
...
`vol_id` is the fork that owns the segment, snapshot, or skeleton file (the *home* volume, which may be an ancestor of the volume the requesting coordinator currently claims). The second URL component is a *segment* ULID for `.idx` / `.prefetch`, a *snapshot* ULID for `.snapshot` / `.manifest` / `.filemap`, and a literal filename for `volume.pub` / `volume.provenance`; the auth pipeline doesn't distinguish — the lineage check only requires `vol_id` to be in the requesting volume's ancestry, and which-flavour membership falls out of step 5 (local file exists). The `/v1/` prefix reserves room for protocol evolution.

For the snapshot routes, the coordinator's claim-time prefetch can also skip the S3 LIST entirely. Marker + manifest names are deterministic from the branch-point snapshot ULID, so a known-branch prefetch fans out two peer GETs in parallel and falls back to keyed S3 GETs (using the canonical `by_id/<vol>/snapshots/<YYYYMMDD>/<snap>{,.manifest}` paths) only on per-artifact peer miss. Filemaps are excluded from prefetch entirely (see "What's served" above). The S3 LIST stays in the listed-path branch (no known branch, e.g. when prefetching a fork's own accumulated snapshots) and as a fallback for any peer-less callers.
