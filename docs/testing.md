...
- **Dedup-across-LBAs + half-promote crash recovery.** Open: `crash_recovery_writemulti_dedup_regression` reproduces the failing 7-op sequence. Two overlapping `WriteMulti` ops produce live entries on different LBAs that share a hash inside one WAL window; after `gc_checkpoint` + `HalfPromotePending` + `gc_apply` + crash, one of the two LBAs reads back zeros instead of the deduped content. Fix tracked on the same branch that added `WriteMulti`.
...
- **Partial-fetch `.present` bit gate and concurrent RMW.** Entries with a clear present bit must fall through to the `SegmentFetcher`; entries with the bit set must be served from the body file. `populate_cache` always sets all bits, so the cleared-bit path is not covered. Closing requires a test `SegmentFetcher` and manual bit clearing. Related: `FetchCoalescer` now serialises the read-modify-write of `<seg>.present` via a per-segment `Mutex<()>` (PR #226) — the bug was lost bits when concurrent disjoint coalescer leases on the same segment ORed against a stale on-disk image. Both correctness properties are concurrency-shaped (single-threaded proptest cannot exercise interleavings); the appropriate vehicles are dedicated multi-threaded tests (the deterministic reproducer landed alongside #226) or a loom model.
